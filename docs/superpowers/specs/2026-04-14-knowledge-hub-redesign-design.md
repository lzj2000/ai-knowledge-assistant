# AI 知识助手改版设计文档

## 概述

本文档定义当前 AI 知识助手的已确认改版方案。
产品仍然主要服务企业内部知识助手场景，但界面也需要足够精致，能够承担轻量对外展示的角色。

本次改版的核心产品判断如下：

- 用户打开产品后，应该可以立即提问。
- 文档上传、文档管理、分类管理都服务于“获得更好的回答”。
- 整体视觉应当更像“AI 编辑台”或“知识中枢”，而不是通用 SaaS 后台，也不是套壳聊天机器人。

## 产品决策

### 主要受众

- 受众是混合型，但明确偏向企业内部用户。
- 界面需要传达可信、结构化、有知识深度的感觉。

### 第一优先动作

- 用户最重要的第一动作是提问。
- 首页必须围绕“快速进入提问”来设计。

### 整体视觉方向

- 已选方向：编辑部感的知识中枢。
- 界面需要显得经过策划、冷静、有判断力。
- 避免常见“AI 产品”视觉陈词滥调，例如紫色渐变、大而空的 hero、千篇一律的聊天气泡。

### 页面角色判断

- 首页是“提问优先”的知识入口页。
- 聊天页是核心工作区。
- 文档页和分类页是知识策展与组织页面。
- 上传页是一个带明确后续引导的知识导入流程。

## 体验原则

- 提问优先：提问始终应该是最快的入口。
- 知识可见：界面需要让用户明确感知回答来自已管理的知识资产。
- 编辑感清晰：回答、来源、元信息、内容排版都应可读、克制、像被编辑过。
- 系统一致：各页面必须像同一套产品，而不是几个拼接起来的 CRUD 界面。
- 企业可信：空状态、状态提示、错误提示都要明确、可控。

## 信息架构

### 全局结构

- 保留全局顶部导航，用于轻量切换主要区域。
- 顶栏不能抢走页面主视觉。
- 全站使用统一的设计语言与布局节奏。

### 响应式策略

- 本次改版必须同时覆盖桌面端与移动端，不把移动端视为“缩小后的桌面页”。
- 桌面端以双栏或多区块布局为主，保留更强的信息并列关系。
- 平板端优先压缩辅助信息区宽度，保持主提问区和主阅读区不失衡。
- 移动端改为纵向堆叠与抽屉式交互，保证首屏仍然围绕“立即提问”展开。
- 聊天工作区在移动端不常驻左侧会话栏，而改为从页面头部按钮唤起的会话抽屉；选择会话后自动收起。
- 首页的知识摘要卡、文档页的筛选区和分类页的知识地图，在移动端都需要改为单列阅读顺序，避免横向压缩后信息失真。

### 路由角色

- `/`：提问优先的知识中枢。
- `/chat`：新对话的主聊天工作区。
- `/chat/[id]`：已有会话的同构聊天工作区。
- `/documents`：知识资产浏览与策展页。
- `/documents/upload`：知识导入页。
- `/categories`：知识地图与分类策展页。

## 视觉系统

### 调性

- 编辑感、结构感、可信感。
- 更像数字化研究桌面或编辑部，而不是传统后台管理系统。

### 色彩方向

- 以带轻微纸张感的浅暖底色替代纯白底。
- 主文本使用深墨色。
- 强调色控制在少量且有记忆点的范围内，以墨蓝和铜橙为主。
- 色彩用于引导层级与状态，而不是单纯装饰。

### 字体方向

- 大标题和关键视觉时刻使用更有气质的衬线字体。
- 正文、控件、密集信息使用清晰的无衬线字体。
- 时间、计数、状态等元信息在需要时可使用更紧凑的工具型样式。
- 本次实现中，栏目级标题与首页主标题统一使用 `Noto Serif SC`。
- 正文、按钮、输入框、列表、表单与说明文本统一使用 `Noto Sans SC`。
- 时间、计数与状态继续使用 `Noto Sans SC`，结合 `tabular-nums` 等排版方式控制秩序感，不额外引入第三套字体。

### 暗色模式边界

- 本次改版不包含用户可切换的暗色模式，也不单独设计 dark 视觉稿。
- 所有全局颜色 token 仍然必须保持语义化命名，例如 `--page`、`--surface`、`--ink`、`--muted`，为后续 dark token 扩展预留空间。
- 如果局部组件需要强调反差，应优先通过层级和边框完成，而不是提前混入未定义的暗色模式样式。

### 组件语言

- 输入框要更大、更有留白、更像编辑台而不是普通搜索框。
- 卡片使用细边框、轻阴影、统一节奏的间距。
- 状态标签、建议问题 chips、来源引用、会话列表项都使用同一套视觉语法。
- 页面头部统一为一套结构：
  标签、标题、简短说明、主操作。

## 页面设计说明

### 首页 `/`

目标：
让用户第一时间发起提问，同时证明系统背后有一套已组织的知识库。

布局：

- 中央主区为大尺寸提问输入区。
- 提问框下方放建议问题 chips。
- 在合适位置放置最近一次会话的快速入口。
- 使用辅助信息块展示知识库规模、最近上传、分类入口等摘要信息。

行为：

- 提问框必须成为首屏主角。
- 辅助信息需要证明系统是“有知识底座的”，但不能与提问入口争主次。
- 即使是空状态，也要让页面显得有用途、有方向。

### 聊天工作区 `/chat` 与 `/chat/[id]`

目标：
作为用户日常提问、阅读、继续历史对话的核心工作区。

布局：

- 左栏为会话历史与新建会话入口。
- 右栏为当前对话主区。
- 对话区顶部展示当前会话标题与轻量上下文提示。
- 对话区底部为大输入区。

消息呈现：

- 用户消息保持紧凑，强调“提问动作”。
- 助手回答以回答卡片形式呈现，而不是普通聊天气泡堆叠。
- 回答卡片必须支持富文本 markdown、引用、代码块、表格、列表等结构。
- 来源作为稳定的引用区显示在回答正文下方。

行为：

- `/chat` 与 `/chat/[id]` 使用同一套页面骨架。
- 新建会话在拿到 `conversationId` 后，需要切换到标准的会话 URL。
- 会话历史不能藏在弱化到难以发现的二级入口里。
- 桌面端会话列表常驻左栏，移动端改为抽屉式侧栏，并保留显式的“最近会话”入口按钮。
- 当前激活会话需要支持删除操作，删除入口放在侧栏会话项的显式操作区或更多菜单中。
- 删除当前会话后，页面需要回退到 `/chat` 的空工作区，不把用户留在失效 URL 上。

### 文档页 `/documents`

目标：
帮助用户理解并浏览当前知识资产，而不只是执行 CRUD 操作。

布局：

- 顶部为带说明文字的强页面头部，并带上传主操作。
- 顶部附近放置筛选与浏览控制。
- 主体采用更有秩序的列表或卡片/列表混合布局。

内容优先级：

- 文档标题。
- 处理状态。
- 所属分类。
- 分块数量。
- 文件元信息。
- 直接发起提问的操作入口。

行为：

- 空状态应当鼓励用户建立第一批知识资产。
- “去提问”操作需要把文档上下文带入聊天流程。

### 上传页 `/documents/upload`

目标：
提供低阻力、高信任感的知识导入体验。

布局：

- 页面中心是主上传面板与拖拽区域。
- 选中文件后，展示标题、分类等辅助字段。
- 下方提供轻量说明，解释支持格式、大小限制和处理流程。

行为：

- 上传区域需要有一定仪式感和聚焦感，不能只是普通表单。
- 上传成功后要明确引导用户去提问。
- 处理进度与处理中状态需要告诉用户接下来会发生什么。

### 分类页 `/categories`

目标：
把分类体系呈现为知识地图，而不只是维护树形结构。

布局：

- 页面头部解释分类对知识组织的作用。
- 主体强调层级、描述与组织意义。
- 新增、编辑仍然可用，但页面整体不能退化成原始后台面板。

行为：

- 分类层级应该一眼可读。
- 页面应当传达“知识组织”感，而不是纯配置页。
- 移动端仍需要能清楚区分父分类与子分类，不能退化为无法阅读的单层列表。

## AI 接入策略

### 已确认方向

- 用 Vercel AI SDK 替换当前手写的聊天请求和自定义 SSE 处理。
- 保留现有应用内的 RAG 检索、上下文拼装、消息持久化职责。
- 聊天路由改为输出 AI SDK 兼容的流式响应。
- 客户端聊天状态与流式渲染由 AI SDK 的 chat hooks 接管。

### 模型接入

- 优先使用 GLM 的 OpenAI 兼容接口。
- 通过 AI SDK 的 OpenAI-compatible provider 接入。
- provider 层保持隔离，后续更换模型细节时不影响页面逻辑。

### Markdown 渲染

- 助手回答使用 `streamdown` 渲染。
- 这样可以在文本仍在流式生成时稳定处理未闭合 markdown 结构。
- 助手消息不应继续依赖普通的非流式 markdown 渲染器。

### 来源呈现

- 来源继续作为助手消息上的结构化数据保存。
- 不依赖模型在正文里自行拼接伪引用。
- UI 中将来源渲染为回答正文下方的独立引用区。

## 聊天数据流

### 服务端流程

1. 校验输入问题。
2. 创建或解析当前会话。
3. 持久化用户消息。
4. 执行检索并构建 RAG 上下文。
5. 通过 AI SDK 启动模型流式生成。
6. 在流式过程中累积完整的助手回答文本。
7. 流结束后一次性保存助手完整回答与来源。

### 客户端流程

1. 立即插入本地用户消息。
2. 将助手回答流式写入当前回答卡片。
3. 生成过程中明确展示输入区状态。
4. 如果流中断，保留已经生成的部分内容。
5. 新会话拿到最终 `conversationId` 后同步更新 URL。

## 状态与交互补充

### 加载状态与骨架屏

- 首页在知识摘要、最近文档、最近会话尚未返回时，展示与正式布局同构的骨架卡片，而不是整页空白或单个 spinner。
- 聊天页会话列表加载时，左栏展示 4 到 6 条骨架会话项；当前对话区若仍在取历史消息，展示回答卡片骨架。
- 文档页与分类页在首屏加载时，分别展示列表骨架与知识地图骨架，明确区分“正在加载”和“暂无数据”。
- 上传页在文件提交后，需要持续显示处理中状态与阶段提示，避免用户误以为上传后页面卡死。

### 键盘交互

- 聊天输入框默认使用 `Enter` 发送消息，`Shift + Enter` 插入换行。
- 中文输入法处于组合输入阶段时，`Enter` 只能确认输入，不得误触发送。
- 移动端保留显式发送按钮，不依赖软键盘行为推断发送。
- 移动端会话抽屉与删除确认弹层应支持 `Esc` 关闭，保持桌面键盘路径一致。

### 会话删除

- 会话删除属于聊天侧栏的一等操作，但不能与“切换会话”竞争主视觉。
- 删除动作必须经过一次明确确认，文案需要说明会同时移除会话及其消息记录。
- 删除成功后，会话项立即从列表移除；若删除的是当前会话，则路由切回 `/chat` 并显示空工作区。
- 删除失败时保留原会话项，并通过 toast 与行内状态提示错误原因。

## 错误处理

- 未检索到内容时，也应渲染一张正常的助手回答卡片，明确说明当前知识库中无相关信息。
- 聊天错误在可行范围内区分网络错误、模型错误、服务端错误。
- 上传与删除这类操作可以继续使用 toast。
- 聊天主流程不能只依赖 toast 传达关键状态。

## 范围边界

### 本次改版范围内

- 当前所有页面的完整视觉改版。
- 将 `/chat` 与 `/chat/[id]` 统一为同一类工作区模式。
- 将聊天流式链路迁移到 Vercel AI SDK。
- 使用 `streamdown` 进行流式 markdown 渲染。
- 复用现有后端数据模型与知识管理能力。

### 本次改版范围外

- 登录、权限或认证系统改造。
- 新的企业治理能力。
- 与聊天持久化或展示元信息无关的大规模数据库重构。
- 将分类扩展为复杂后台子系统。

## 验证要求

- 验证聊天路由的流式响应行为与消息持久化行为。
- 验证聊天页在新会话与已有会话两种情况下的渲染与切换。
- 验证流式 markdown 渲染与来源区的稳定性。
- 运行 lint 与生产构建，确认与 Next.js 16.2.3 和 React 19.2.4 兼容。

## 实现约束说明

- 路由处理必须继续遵循 App Router 下 `app/api/**/route.ts` 的约定。
- 页面与布局修改必须遵循已从本地文档确认过的 Next.js 16 App Router 规则。
- 改版可以保留仍然合理的信息架构，但不应保留当前低完成度的视觉表达。

## 成功标准

- 首次进入产品的用户能够毫不犹豫地从首页发起提问。
- 界面能让知识库显得真实、可见、可信。
- 聊天工作区成为产品中心。
- 文档与分类页面更像被策展过的知识空间，而不是简单管理页。
- 助手回答能够平滑流式输出，并在生成过程中稳定渲染 markdown。
